...
export async function POST(request: Request) {
  ...
  try {
    ...
    // if user exist with username and verified
    ...
    if (existingUserByEmail) {
      ...
      // user not verified. then update password, verifyCode, verifyCodeExpiry
      ...
      // if user not exist with email, then create new user
      const hashedPassword = await bscrypt.hash(password, 10);
